...
async fn main() {
    ...
    match matches.subcommand() {
        Some(("trainings", matches)) => {
            let path = matches.get_one::<String>("path").unwrap();
            let file = std::fs::read_to_string(path).unwrap();
            let style = toml::from_str::<Style>(&file).unwrap();

            let trainings = sqlx::query!("SELECT COUNT(id) FROM trainings")
                .fetch_one(&db)
                .await
                .unwrap();
            ...
            let trainings =
                sqlx::query!("SELECT id, name, description, weight, times, tags FROM trainings",)
                    .fetch_all(&db)
                    .await
                    .unwrap();
